...
Loads the Mobile ALOHA URDF (from `assets/urdf/`, fetched/expanded by
`scripts/fetch_urdf.sh`) into rerun using rerun's built-in URDF loader (rerun-sdk >= 0.29).
...
URDF source: Interbotix/Trossen `aloha` description (xacro -> URDF; see
scripts/fetch_urdf.sh). MJCF alternative: google-deepmind/mujoco_menagerie `aloha`.
...
DEFAULT_URDF = Path("assets/urdf/aloha.urdf")
